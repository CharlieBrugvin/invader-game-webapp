...
export class InvaderColumnUtils {
    ...
    public static addInvader(invaders: Invader[][], columnIndex: number = null): Invader[][] {
        ...
        let newInvaders = invaders.map( (column, index) =>
            index === columnIndex ? 
                this.addInvaderToColumn(column)
               : column
        )
    ...
    public static addInvaderToColumn(column: Invader[]): Invader[] {
        // we find the the topest invader
        let invaderTopest: number = 0;
        column.forEach(invader => {
            if (invader['top.%'] < invaderTopest)
                invaderTopest = invader['top.%']; 
            }
        )
        ...
        return [
            ...column,
            InvaderUtils.create(invaderTopest - appSettings.invader['height.%'] - 1)
        ]
    }
}